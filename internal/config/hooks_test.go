...
func TestLoadBuiltinHooksParsesDevelopmentWorkCompleteTrigger(t *testing.T) {
	dir := t.TempDir()
	writeHookFile(t, dir, "a.yaml", `
version: 1
id: development-work-complete-hook
triggers:
...
`)
	...
	if catalog.Hooks[0].Triggers[0] != HookTriggerDevelopmentWorkComplete {
		t.Fatalf("unexpected trigger: %q", catalog.Hooks[0].Triggers[0])
	}
}
